...
export default function() {
  return pump(
    ...
    source(`${config.env}.js`), // Convert from Browserify stream to vinyl stream.
    buffer(), // Convert from streaming mode to buffered mode.
    injectEnvironment(), // Replace templated #env{var} vars.
    gulpif(config.sourcemaps, sourcemaps.init({ loadMaps: true })),
    gulpif(config.minify, uglify({ mangle: false })),
    rev(),
    gulpif(config.sourcemaps, sourcemaps.write("./", { sourceRoot: "/sourcemaps" })),
    gulp.dest(path.join(config.buildDir, config.scriptsDir))
  );
}
